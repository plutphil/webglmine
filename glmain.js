...
function main() {
  ...
  chunkWorker.onmessage = e => {
    //const ch = chman.getChunk(e.data.x,e.data.y,e.data.z);
    const chdata = e.data[0];
    //Object.assign(ch1, chdata);
    //console.log(e)
    ch = chman.getChunk(chdata.x,chdata.y,chdata.z);
    Object.assign(ch, chdata);
    //ch1.cube = Object.assign(new ChunkMesh(), e.data[1]);
    ch.cube = Object.assign(new ChunkMesh(), e.data[1]);
    //ch1.meshgen = true;
    //ch.cube=;
    //ch.cube.clear();
  }
  chunkWorker.postMessage(ch1);
  /*chunkMeshWorker.onmessage=e=>{
    console.log(e);
    const ch = chman.getChunk(e.data.x,e.data.y,e.data.z);
    Object.assign(ch, e.data);
    ch.cube.clear();
  };*/
  ...
  */
  ...
  function resizeCanvas() {
    ...
    canvas.height = window.innerHeight;
    gl.viewport(0, 0, gl.canvas.width, gl.canvas.height);
    mat4.perspective(camera.matrixprojection, degToRad(50), gl.canvas.clientWidth / gl.canvas.clientHeight, 0.1, 1000);
  ...
  chunkRenderer = new ChunkRenderer(shaders.getCubeShader(),texatlas);
  chunkRenderer.setattributes();
  ...
  function update() {
    ...
    if (ch1.meshgen) {
      if (ch1.stored) {
        chunkRenderer.draw(gpumesh, mvp)
      } else {
        chunkRenderer.store(ch1.cube, gpumesh);
        ch1.stored=true;
      }
    }
    //ch.draw(mvp);
